...
Param(
  ...
  [Alias("FilePath")]
  [string] $Name = "Temp.cab",

  [Parameter(HelpMessage = "Source folder to items to include in cabinet file.", Position = 0, Mandatory = $true, ValueFromPipelineByPropertyName = $true)]
...
.Set MaxDiskSize=0
.Set DestinationDir=$destination;
"@
  #endregion

  #region body
  $message = "Writing ddfheader t0 $ddfFile"
  Write-Host $message
  Write-Debug $message
...
Function New-Cab($file) {
  ...
  if ($debug) { 
    makecab /f $file /V3 /L $destination 
  }
  Else { 
    makecab /f $file /L $destination 
  }
  #endregion
} 
New-Cab $ddfFile